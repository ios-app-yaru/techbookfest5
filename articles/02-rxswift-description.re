...
ボタンをタップするとアラートを表示、のようなインタラクティブなシステムや通信処理、アニメーションのようにダイナミックに状態が変化するようなシステムに対して宣言的に動作を記述することができるため、よく使われます。
逆に、サーバーサイド側ではそのような必要は少ないため、フロントエンド側と比べて使われることは少ないです。
